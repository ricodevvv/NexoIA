...
const runPythonSpec: ToolSpec = {
  name: "run_python",
  description:
    "Ejecuta código Python 3 en un sandbox aislado y devuelve stdout, stderr y el valor de la última expresión. Úsalo para cálculos exactos, análisis de datos, transformar archivos o generar gráficas. Hay numpy, pandas, matplotlib, scipy, sympy y scikit-learn. Los adjuntos del chat están en el directorio de trabajo (/mnt/data) con su nombre original, incluidos .zip, .tar.gz, Excel y Word: ábrelos con zipfile, tarfile, openpyxl, etc. Las figuras de matplotlib se guardan solas; cualquier archivo que escribas en /mnt/output se entrega al usuario como descarga (máximo 10 archivos y 10 MB en total). Si armas varios archivos o un proyecto, ponlos en una carpeta dentro de /mnt/output: se entrega sola como .zip. También puedes crear el .zip tú con zipfile o shutil.make_archive. No hay acceso a internet ni instalación de paquetes, y cada ejecución empieza de cero (no se conserva estado entre llamadas).",
  inputSchema: {
