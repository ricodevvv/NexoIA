...
const runPythonSpec: ToolSpec = {
  name: "run_python",
  description:
    "Ejecuta código Python 3 en un sandbox aislado y devuelve stdout, stderr y el valor de la última expresión. Úsalo para cálculos exactos, análisis de datos, transformar archivos o generar gráficas. Hay numpy, pandas, matplotlib, scipy, sympy y scikit-learn. Los adjuntos del chat están en el directorio de trabajo (/mnt/data) con su nombre original, incluidos .zip, .tar.gz, Excel y Word: ábrelos con zipfile, tarfile o pandas, u openpyxl y python-docx después de instalarlos con micropip. Las figuras de matplotlib se guardan solas. Todo archivo que crees o modifiques se le entrega al usuario como descarga al terminar la ejecución, esté en /mnt/output o en /mnt/data: si hay .zip o .tar.gz se entregan esos, si hay pocos archivos sueltos se entregan tal cual y si hay una estructura de carpetas se empaqueta sola en archivos.zip (máximo 10 archivos y 10 MB en total). Para entregar un proyecto, créalo y comprímelo en la misma ejecución. Nunca pegues un archivo en base64 ni su contenido completo en tu respuesta: el usuario ya lo tiene para descargar. Hay acceso a internet (solo HTTP y HTTPS a direcciones públicas): instala paquetes de PyPI con `import micropip; await micropip.install(['openpyxl', 'python-docx'])` (sirven los de Python puro y los que trae Pyodide, no los que compilan C), y descarga datos con `requests` o con `from pyodide.http import pyfetch; r = await pyfetch(url)`. `urllib.request` no funciona con HTTPS. Los paquetes de Pyodide como lxml o pillow se cargan solos con el import. No hay pip ni subprocesos, la ejecución dura como máximo 120 segundos y cada una empieza de cero: lo instalado o descargado no se conserva entre llamadas.",
  inputSchema: {
